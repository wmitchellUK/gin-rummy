-- A profile exists for every player identity, but a public name is optional
-- until that player creates or joins a game.
...
-- New private links are high-entropy bearer capabilities. Only a SHA-256
-- digest is persisted; browser-readable table grants remain unchanged.
alter table public.games add column invite_token_digest text;
create unique index games_invite_token_digest_key
  on public.games(invite_token_digest) where invite_token_digest is not null;

-- Membership may allow a player to read a game's public lifecycle metadata,
-- never either legacy codes or new token digests.
revoke select on public.games from authenticated;
grant select (id, status, rules, created_by, source_game_id, rematch_requested_by,
  created_at, started_at, completed_at, last_activity_at) on public.games to authenticated;

create or replace function public.create_waiting_game(
  p_invite_code citext, p_invite_token_digest text, p_creator_id uuid, p_display_name text, p_rules jsonb
)
returns uuid
...
begin
  insert into public.games(id, invite_code, invite_token_digest, status, rules, created_by)
    values (v_game_id, p_invite_code, p_invite_token_digest, 'WAITING', p_rules, p_creator_id);
...
create or replace function public.join_game_and_start_with_invite_token(
  p_invite_token_digest text, p_user_id uuid, p_display_name text, p_next_state jsonb, p_events jsonb
)
returns table(outcome text, game_id uuid, version integer)
language plpgsql security invoker set search_path = public
as $$
declare v_game public.games%rowtype; v_action_id uuid := gen_random_uuid(); v_event jsonb; v_index integer := 0;
begin
  select * into v_game from public.games where invite_token_digest = p_invite_token_digest for update;
  if not found then raise exception 'INVITE_UNAVAILABLE' using errcode = 'P0001'; end if;
  if exists (select 1 from public.game_players where game_players.game_id = v_game.id and user_id = p_user_id) then
    return query select 'ALREADY_A_PLAYER'::text, v_game.id, null::integer;
    return;
  end if;
  if v_game.status <> 'WAITING' then
    return query select 'FULL'::text, null::uuid, null::integer;
    return;
  end if;
  ...
  update public.game_state as checkpoint set version = 1, canonical_state = p_next_state, updated_at = now()
  ...
  return query select 'JOINED'::text, v_game.id, 1;
end;
$$;

revoke all on function public.create_waiting_game(citext, text, uuid, text, jsonb) from public, anon, authenticated;
revoke all on function public.join_game_and_start_with_invite_token(text, uuid, text, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.create_waiting_game(citext, text, uuid, text, jsonb) to service_role;
grant execute on function public.join_game_and_start_with_invite_token(text, uuid, text, jsonb, jsonb) to service_role;
