...
begin
  ...
  if found then
    if existing.game_id <> p_game_id or existing.actor_id <> p_actor_id then
  ...
  insert into public.game_actions(action_id, game_id, actor_id, expected_version, action_type, accepted_version)
    values (p_action_id, p_game_id, p_actor_id, p_expected_version, p_action_type, p_expected_version + 1);
  for event in select value from jsonb_array_elements(p_events) loop
    insert into public.game_events(game_id, state_version, action_id, event_index, event_type, visibility, recipient_user_id, payload)
      values (p_game_id, p_expected_version + 1, p_action_id, event_index, event->>'type',
        ...
        case when event->'visibility'->>'kind' = 'PLAYER' then event->'visibility'->>'playerId' else null end,
        event);
...
revoke all on function public.commit_game_action(uuid, uuid, uuid, integer, text, jsonb, public.game_status, jsonb, jsonb) from public, anon, authenticated;
grant execute on function public.commit_game_action(uuid, uuid, uuid, integer, text, jsonb, public.game_status, jsonb, jsonb) to service_role;
