...
export type LoadedGame = { readonly state: GameState; readonly snapshots: readonly PlayerSnapshot[]; readonly status: string };
...
export async function loadCanonicalGame(gameId: string): Promise<LoadedGame> {
  ...
  const [{ data: checkpoint, error: checkpointError }, { data: players, error: playersError }, { data: game, error: gameError }] = await Promise.all([
    ...
    admin.from("games").select("status").eq("id", gameId).maybeSingle(),
  ]);
...
export async function createGame(inviteCode: string, creatorId: string, displayName: string, rules: object): Promise<string> {
  const { data, error } = await createAdminClient().rpc("create_waiting_game", {
    p_invite_code: inviteCode, p_creator_id: creatorId, p_display_name: displayName, p_rules: rules,
  });
  if (error) {
