...
deadwood, layoffs, declaration, final discard, score, and next-hand readiness. A
cancelled result includes its reason and scores but neither hand. Game-complete data
projects every completed hand through that same safe result shape so participants can
review prior scored hands; it never returns canonical state or cards from cancelled
hands.
