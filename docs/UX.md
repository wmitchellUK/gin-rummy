...
Single-player uses the same table. The opponent is Naia, identified in text as a computer opponent and represented by a restrained painted PomeraNaian portrait. The portrait is decorative beside the accessible name. Naia has no dialogue, taunts, mascot animation, or casino styling. Her opening/draw and discard decisions appear as separate, short thinking beats so public actions remain understandable.
...
| `CardHand` | Maintains a local, non-authoritative per-game display order that survives refresh and remembers encountered card identities across later deals. Supports selection, a shallow fan, pointer/touch drag, arrow-key navigation, and Shift+Arrow reorder throughout active play. Server-projected meld candidates are labeled only when their cards are contiguous in that private order; only exact member cards receive a meld border, with a compact Run/Set label above and a subtle center-bounded rail below the group rather than an enclosing frame. |
...
| `GameResult` | Final modal with a restrained victory crest, winner, final scores, reviewable hand history, and rematch request/accept/decline state. Each scored history entry opens the fully resolved tabletop with optional layoff replay; cancelled entries explain the cancellation without revealing cards. No game-play controls remain active. |
...
| Reorder | Throughout active play, regardless of whose turn it is, horizontal pointer/touch drag reorders cards after a movement threshold; Shift+Arrow provides the keyboard equivalent. Suppress native text/image selection and browser dragging within the hand. Persist the private preference locally, retain absent card identities for later deals, and append unseen cards in server order. Reordering never sends a game action or changes scoring. |
| Meld organization | The server projects every rule-valid candidate from the caller’s own hand. Highlight only the exact member cards of candidates whose cards are adjacent in the private display order, place compact Run/Set labels above those groups, and connect each group with a faint gold rail below the cards that stops at the first and last member centers. Never draw an enclosing frame; collapse contained same-kind subgroups to the longest label and retain genuine Run/Set overlaps. Labels and rails are cosmetic and never replace server discard, knock, gin, or deadwood decisions. |
...
| Naia action | Wake one server action at a time after a short phase-specific delay. On refresh, restart the pending wakeup from the latest projected version. Duplicate tabs and stale wakeups must remain harmless. |
| Hand reveal / scoring | On server transition to hand complete, disable table controls and open the viewer-relative `HandResolutionTable`. Announce and highlight the declaration/final discard, reveal both safely projected hands in Meld and Deadwood groups, then animate each server-projected layoff onto its target meld while updating deadwood. Finish with an explicit Knock, Gin, or Undercut result, calculation, score, readiness, and next-hand control. Show result skips presentation only; Replay layoffs sends no game action. |
...
| Match completion | Play the same deciding-hand resolution and keep its outcome visible until the player selects View match summary; never advance on a timer. Keep Replay deciding hand beside the winner, final score, interactive hand history, and rematch controls. Historical scored hands open already resolved with Replay layoffs and Back to match summary; cancelled hands show no cards. Show rematch request status and link to the new match only after mutual acceptance. |
