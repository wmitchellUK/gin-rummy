...
| `GameResult` | Final modal with a restrained victory crest, winner, final scores, the complete deciding-hand reveal, completed-hand summary, and rematch request/accept/decline state. No game-play controls remain active. |
...
| Match completion | Play the same deciding-hand resolution before transitioning to the scrollable `GameResult`. Keep Replay deciding hand beside the winner, final score, hand history, and rematch controls. Show rematch request status and link to the new match only after mutual acceptance. |
