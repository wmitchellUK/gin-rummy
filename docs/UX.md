...
| Match completion | Replace hand continuation with one scrollable `GameResult` that celebrates the winner and preserves the full deciding-hand reveal before hand history and rematch controls. Show rematch request status and link to the new match only after mutual acceptance. |
