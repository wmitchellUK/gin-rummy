  ...
  return <main className="game-shell"><section className="game-table" aria-live="polite">
    ...
    {game.status === "WAITING" ? <WaitingGame inviteUrl={inviteUrl} /> : gameplayControlsAreAvailable(game) ? <>
      <section className="opponent-area" aria-label="Opponent">
        <div className="player-identity opponent-identity"><div className="avatar" aria-hidden="true">{initials(game.opponent?.displayName ?? "?")}</div><div><p className="eyebrow">Opponent <span className="connection"><i /> At table</span></p><h1>{game.opponent?.displayName ?? "Opponent"}</h1><p className="seat-note">{game.opponent?.cardCount ?? 0} cards {game.dealerId ? "· Dealer" : ""}</p></div></div>
      ...
      <section className="player-area" aria-labelledby="your-hand">
        <div className="player-hand-heading"><div className="player-identity"><div className="avatar you-avatar" aria-hidden="true">{initials(game.you.displayName)}</div><div><p className="eyebrow">You {youAreActive ? "· Your turn" : "· At the table"}</p><h2 id="your-hand">{game.you.displayName}</h2></div></div><div className="hand-tools"><span>{game.you.hand.length} cards</span><button className="sort-button" onClick={sortHand}>Sort <span aria-hidden="true">↕</span></button></div></div>
...
function OpponentHand({ count }: { count: number }) { return <div className="opponent-hand" aria-label={`Opponent has ${count} cards`}>{Array.from({ length: count }, (_, index) => <CardBack key={index} />)}</div>; }
...
function CardFace({ card, marker }: { card: PublicCard; marker?: "Hold" | "Drawn" }) { return <span className={`card-face ${suitColorClass(card)}`} aria-hidden="true"><CardMark card={card} />{marker && <span className="turn-card-badge">{marker}</span>}<span className="card-face-suit">{SUIT_SYMBOLS[card.suit]}</span><span className="card-mark card-mark-inverted"><span>{card.rank}</span><span className="suit-icon">{SUIT_SYMBOLS[card.suit]}</span></span></span>; }
